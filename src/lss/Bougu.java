...
public class Bougu implements Common {
    ...
    public String getText() {
        ...
        if (op.HIT_CLOSE + op2.HIT_CLOSE > 0) {
            text += " 攻撃成功+" + (op.HIT_CLOSE + op2.HIT_CLOSE);
        }
        if (op.HIT_FAR + op2.HIT_FAR > 0) {
            text += " 弓命中率+" + (op.HIT_FAR + op2.HIT_FAR);
        }
        ...
        if (op.element_resisit[FIRE] > 0) {
            text += " 火の属性" + op.element_resisit[FIRE];
        }
        if (op.element_resisit[WATER] > 0) {
            text += " 水の属性" + op.element_resisit[WATER];
        }
        if (op.element_resisit[WIND] > 0) {
            text += " 風の属性" + op.element_resisit[WIND];
        }
        if (op.element_resisit[EARTH] > 0) {
            text += " 地の属性" + op.element_resisit[EARTH];
        }
        if (op.MR > 0) {
        ...
        if (type.equals("リング") || type.equals("アミュレット")
            ...
            if (tokusei.equals("根性")) {
                switch (enchant) {
                    ...
                    case 5:
                        op2.HP = 40;
                        op2.effect = "ポーション回復量 +2%";
                        break;
                    case 6:
                        op2.HP = 40;
                        op2.effect = "ポーション回復量 +4%";
                        break;
                    case 7:
                        op2.HP = 50;
                        op2.effect = "ポーション回復量 +6%";
                        break;
                    case 8:
                        op2.HP = 50;
                        op2.effect = "ポーション回復量 +8%";
                        break;
                }
            ...
            if (name.equals("ルームティス ブルー イアリング")) {
                switch (enchant) {
                    case 0:
                        op.effect = "POT回復量 +2% +2";
                        break;
                    case 1:
                        op.effect = "POT回復量 +6% +6";
                        break;
                    case 2:
                        op.effect = "POT回復量 +8% +8";
                        break;
                    case 3:
                        op.effect = "POT回復量 +10% +10";
                        break;
                    case 4:
                        op.effect = "POT回復量 +12% +12";
                        break;
                    case 5:
                        op.AC = -1;
                        op.effect = "POT回復量 +14% +14";
                        break;
                    case 6:
                        op.AC = -2;
                        op.effect = "POT回復量 +16% +16";
                        break;
                    case 7:
                        op.AC = -2;
                        op.effect = "POT回復量 +18% +18";
                        break;
                    case 8:
                        op.AC = -3;
                        op.effect = "POT回復量 +20% +20";
                        break;
                }
            }
        } else {
